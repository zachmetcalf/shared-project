...
powershell -NoProfile -Command "$paths = @('.codex','.github','.vscode','config','data','docs','infra','scripts','services','source') | Where-Object { Test-Path $_ }; if ($paths.Length -gt 0) { Compress-Archive -Update -Path $paths -DestinationPath '%zippath%' }"
