...
powershell -NoProfile -Command "$paths = @('.github','.vscode','docs','scripts') | Where-Object { Test-Path $_ }; if ($paths.Length -gt 0) { Compress-Archive -Update -Path $paths -DestinationPath '%zippath%' }"
